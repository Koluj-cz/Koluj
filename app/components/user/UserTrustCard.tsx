...
export default function UserTrustCard({
  ...
  const dialog =
    open && mounted
      ? createPortal(
          <div
            className="fixed inset-0 z-[5000] flex items-center justify-center bg-black/65 p-4"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="max-h-[calc(100dvh-32px)] w-full max-w-2xl overflow-y-auto rounded-[28px] bg-white p-5 pb-[calc(24px+env(safe-area-inset-bottom))] shadow-2xl md:p-8">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  ...
                  className="shrink-0 rounded-full p-2 hover:bg-[var(--koluj-bg)]"
                  aria-label="Zavřít"
                >
                  <X size={22} />
                </button>
...
function MissingRequirements({ trust }: { trust: UserTrustSummary }) {
  ...
  if (target.missingCompletedBookings > 0) {
    items.push(`Dokončit ještě ${target.missingCompletedBookings} rezervací`);
  }
  if (target.missingRatings > 0) {
    items.push(`Získat ještě ${target.missingRatings} hodnocení`);
  }
  ...
  if (!items.length) {
    items.push("Podmínky jsou splněné. Úroveň se aktualizuje automaticky.");
  }

  return (
    <ul className="mt-3 space-y-2 text-sm font-bold text-[var(--koluj-muted)]">
      {items.map((item) => (
        <li key={item}>• {item}</li>
      ))}
    </ul>
