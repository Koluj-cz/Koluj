...
export default function OfferDetailClient({
  ...
  const [item] = useState<ItemDetail | null>(initialItem);
  const [images] = useState<ItemImage[]>(initialImages);
  const [selectedImage, setSelectedImage] = useState(
    initialItem.primary_image_url || initialImages[0]?.image_url || "",
  );
  const [availabilityBlocks] = useState<AvailabilityBlock[]>(
    initialAvailabilityBlocks,
  );
